    ...
    for (const file of files) {
        ...
        if (stat.isDirectory()) {
            ...
            content = content.replace(/border-brand-cyan\/10 hover:border-brand-cyan\/50/g, () => {
                cardCount++;
                return cardCount % 2 === 0 ? 'border-brand-pink/20 hover:border-brand-pink/60' : 'border-brand-cyan/20 hover:border-brand-cyan/60';
            });
            
            // Rebalance the title screen (App.tsx mostly) icons
            content = content.replace(/<span className="(.*?)text-brand-cyan(.*?)">(.*?)<\/span>/g, '<span className="$1text-brand-pink$2">$3</span>');
            ...
            content = content.replace(/text-3xl font-black text-brand-cyan/g, 'text-3xl font-black text-brand-pink');
            content = content.replace(/text-5xl font-black text-brand-cyan/g, 'text-5xl font-black text-brand-pink');
