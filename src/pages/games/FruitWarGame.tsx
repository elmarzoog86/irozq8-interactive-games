        ...
        <div className="flex-1 flex flex-col gap-6">
          {gameState === 'lobby' && (
            <div className="flex-1 bg-zinc-900/50 rounded-2xl border border-cyan-500/20 p-8 flex flex-col items-center justify-center ">
              <div className="w-24 h-24 bg-red-500/10 rounded-full flex items-center justify-center mb-8 border border-red-500/30 glow-cyan">
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className={`px-4 py-2 rounded-lg font-arabic text-sm transition-colors ${
                        gameMode === 'roulette' ? 'bg-cyan-500 text-brand-black font-bold' : 'bg-zinc-700 text-zinc-300'
                      }`}
                    >
                      الروليت
                    </button>
                    <button 
                      ...
                      className={`px-4 py-2 rounded-lg font-arabic text-sm transition-colors ${
                        gameMode === 'voting' ? 'bg-cyan-500 text-brand-black font-bold' : 'bg-zinc-700 text-zinc-300'
                      }`}
                    >
                      التصويت
                    </button>
          ...
          {gameState === 'roulette' && (
            <div className="flex-1 bg-zinc-900/50 rounded-2xl border border-cyan-500/20 p-8 flex flex-col items-center justify-center ">
              <h2 className="text-3xl font-bold font-arabic mb-8 text-white">
                عجلة <span className="text-cyan-500 glow-cyan-text">الروليت</span>
              </h2>
              
              {/* Placeholder for Roulette Wheel */}
              <div className="w-96 h-96 rounded-full border-8 border-cyan-500/30 flex items-center justify-center bg-zinc-800 relative overflow-hidden glow-cyan">
                <div className="absolute inset-0 bg-[conic-gradient(from_0deg,#00e5ff_0deg_36deg,#0a0a0a_36deg_72deg,#00e5ff_72deg_108deg,#0a0a0a_108deg_144deg,#00e5ff_144deg_180deg,#0a0a0a_180deg_216deg,#00e5ff_216deg_252deg,#0a0a0a_252deg_288deg,#00e5ff_288deg_324deg,#0a0a0a_324deg_360deg)] opacity-20" />
                <span className="text-6xl z-10">🎡</span>
              </div>

              <div className="mt-12 flex gap-4">
                <button className="bg-cyan-500 hover:bg-cyan-400 text-brand-black px-8 py-3 rounded-full font-bold font-arabic transition-colors glow-cyan">
                  لف العجلة
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 bg-zinc-900/50 rounded-2xl border border-cyan-500/20 p-8 flex flex-col ">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-3xl font-bold font-arabic text-white">
                  مرحلة <span className="text-cyan-500 glow-cyan-text">التصويت</span>
                </h2>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 flex-1 overflow-y-auto p-2 custom-scrollbar">
                {players.filter(p => p.isActive).map(player => (
                  <div key={player.id} className="bg-zinc-800/80 border border-zinc-700 rounded-xl p-4 flex flex-col items-center gap-3 relative overflow-hidden group hover:border-cyan-500/50 transition-colors">
                    ...
                    <span className="font-bold font-arabic text-cyan-500">{player.fruit}</span>
          ...
          {gameState === 'results' && (
            <div className="flex-1 bg-zinc-900/50 rounded-2xl border border-cyan-500/20 p-8 flex flex-col items-center justify-center ">
              ...
              <h2 className="text-4xl font-bold font-arabic mb-8 text-white">
                الفائز <span className="text-cyan-500 glow-cyan-text">النهائي</span>
              </h2>
              
              <div className="text-center">
                <span className="text-6xl mb-4 block">👑</span>
                <span className="text-3xl font-bold text-white block mb-2">PlayerOne</span>
                <span className="text-xl text-cyan-500 font-arabic">بطل حرب الفواكه!</span>
              </div>
