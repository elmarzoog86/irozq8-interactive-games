...
export function RussianRouletteChatGame({ messages = [], onLeave, channelName, isConnected, error }: GameProps) {
  ...
  const startGame = () => {
    if (players.length < 2) {
       setMessage('نحتاج لاعبين على الأقل لبدء الجولة!');
       return;
    }
  ...
  }, [mode, activePlayerIndex, players, eliminatePlayer, soundEnabled]);
  ...
  useEffect(() => {
    ...
    newMsgs.forEach(msg => {
      ...
      if (mode === 'playing') {
         if (text.startsWith('!bet ') && !players.includes(uname) && !deadPlayers.includes(uname)) {
            const target = text.replace('!bet ', '').replace('@', '').trim();
            if (players.includes(target)) {
               setBets(b => ({ ...b, [uname]: target }));
            }
         }
         ...
         if (players[activePlayerIndex] === uname) {
            if (text === '!pull') {
               if (bullets.includes(currentChamber)) {
                  eliminatePlayer(uname, 'تلقى رصاصة!');
               } else {
  ...
  }, [messages, mode, players, deadPlayers, activePlayerIndex, currentChamber, bullets, eliminatePlayer, surviveTurn, soundEnabled]);
      ...
      <div className="flex-1 rounded-[40px] border border-zinc-800 bg-black backdrop-blur-xl flex flex-col relative overflow-hidden shadow-[inset_0_0_100px_rgba(0,0,0,0.9)]">
        ...
        <AnimatePresence>
          {mode === 'lobby' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -20 }} className="absolute inset-0 z-20 flex flex-col items-center justify-center p-8 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-zinc-900 to-black overflow-y-auto">
              ...
              <p className="text-zinc-500 text-xl font-medium mb-8 max-w-3xl text-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 max-w-4xl mb-8 w-full">
                ...
                <div className="bg-black/80 border border-red-900/50 p-4 rounded-2xl">
                   <div className="font-bold text-red-500 mb-1 text-lg">!pull</div>
                   <div className="text-zinc-400 text-sm">أثناء دورك: اسحب الزناد. إمّا أن تنجو أو تموت!</div>
                </div>
                <div className="bg-black/80 border border-zinc-800 p-4 rounded-2xl">
                   <div className="font-bold text-white mb-1 text-lg">!pass @اسم</div>
                   <div className="text-zinc-400 text-sm">إذا سحبت الزناد ونجوت، يمكنك تمرير المسدس للاعب معين.</div>
                </div>
                <div className="bg-black/80 border border-zinc-800 p-4 rounded-2xl">
                   <div className="font-bold text-white mb-1 text-lg">!spin</div>
                   <div className="text-zinc-400 text-sm">للف أسطوانة المسدس وتغيير مكان الرصاصة قبل الإطلاق.</div>
                </div>
                <div className="bg-black/80 border border-zinc-800 p-4 rounded-2xl">
                   <div className="font-bold text-white mb-1 text-lg">!skip</div>
                   <div className="text-zinc-400 text-sm">إذا خفت من سحب الزناد، تخطى دورك! لكن سيتم إضافة رصاصة إضافية كعقاب للجميع!</div>
                </div>
                <div className="bg-black/80 border border-zinc-800 p-4 rounded-2xl">
                   <div className="font-bold text-white mb-1 text-lg">!bet @اسم</div>
                   <div className="text-zinc-400 text-sm">للجمهور غير المشارك: راهن على من سيموت تالياً!</div>
                </div>
              </div>
        ...
        {mode === 'playing' && (
          <div className="flex-1 flex flex-col items-center justify-center p-6 overflow-hidden z-10 relative">
             ...
             <div className="w-full max-w-3xl mt-4 bg-zinc-900/30 border border-zinc-800 rounded-3xl p-6 z-10 hidden lg:block">
                <h3 className="text-zinc-500 mb-4 font-bold">المراهنات من الجمهور</h3>
                <div className="flex flex-wrap gap-3 max-h-24 overflow-y-auto custom-scrollbar">
                   {Object.entries(bets).map(([better, target]) => (
                      <span key={better} className="px-3 py-1 rounded border bg-black border-zinc-800 text-zinc-400 text-sm">
                         <span className="text-white">{better}</span> يراهن على موت <span className="text-red-500">{target}</span>
                      </span>
                   ))}
                   {Object.keys(bets).length === 0 && <span className="text-zinc-700 text-sm">لا يوجد مراهنات بعد... (اكتب !bet @اسم)</span>}
                </div>
             </div>
