  ...
  return (
    <div className="flex h-full w-full gap-6 p-6 min-h-0 font-arabic text-white" dir="rtl">
      ...
      <div className="flex-1 bg-brand-black/80 rounded-[40px] border border-brand-cyan/20 overflow-hidden shadow-2xl flex flex-col relative">
        ...
        <div className="flex justify-between items-start mb-8 shrink-0">
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="flex gap-2">
              <button 
                onClick={onLeave}
                className="bg-brand-black/70 border border-brand-cyan/20 px-4 py-2 rounded-xl text-sm font-bold text-zinc-400 hover:text-white transition-all flex items-center gap-2"
              >
                <XCircle className="w-4 h-4" /> خروج
              </button>
              <div className="bg-brand-black/70 border border-brand-cyan/20 p-2 rounded-xl flex items-center gap-3">
                <code className="text-brand-cyan font-bold px-2">{window.location.origin}/howmany/{roomId}</code>
                <button 
                  onClick={copyLink}
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-brand-cyan/60 text-sm">استخدم الرابط أعلاه للانضمام للمنافسة</p>
              </div>
            </div>
          </motion.div>
        )}
        ...
        <AnimatePresence mode="wait">
                ...
                <div className="mt-12 grid grid-cols-4 gap-4 max-w-2xl mx-auto">
                  {state.players.map(p => (
                    <motion.div 
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="bg-brand-black/70 border border-brand-cyan/10 p-4 rounded-2xl flex flex-col items-center gap-2"
                    >
                      <div className="w-12 h-12 bg-brand-cyan/20 rounded-full flex items-center justify-center text-xl">
                        👤
                      </div>
          ...
          {state.status === 'matchmaking' && (
            ...
            >
              <h2 className="text-6xl font-black italic text-brand-cyan">من سيواجه من؟</h2>
              <div className="flex items-center gap-12">
                <div className="bg-brand-black/70 border-2 border-brand-cyan/50 p-12 rounded-[40px] w-80 shadow-2xl">
                  <div className="text-8xl mb-6">👤</div>
                  <h3 className="text-3xl font-black">{state.players.find(p => p.id === state.currentMatch?.[0])?.name}</h3>
                </div>
                <Swords className="w-24 h-24 text-brand-cyan animate-pulse" />
                <div className="bg-brand-black/70 border-2 border-brand-cyan/50 p-12 rounded-[40px] w-80 shadow-2xl">
                  <div className="text-8xl mb-6">👤</div>
            ...
            >
              <div className="bg-brand-indigo/10 border border-brand-indigo/20 p-8 rounded-3xl inline-block mb-8">
               ...
               <div className="bg-brand-black/70 border border-brand-cyan/20 p-8 rounded-3xl min-w-[300px]">
                  ...
                  {state.bid > 0 ? (
                    <div>
                      <div className="text-6xl font-black text-white glow-cyan-text mb-2">{state.bid}</div>
                      <div className="text-brand-cyan text-lg">
                  ...
                  <div className="mt-6 pt-6 border-t border-brand-cyan/10">
                    <p className="text-sm text-zinc-400 mb-2">الدور الحالي</p>
                    <div className="text-2xl font-black text-brand-cyan animate-pulse">
               ...
               {socket && state.turn === socket.id && (
                <motion.div 
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="mt-8 bg-brand-black/80  p-6 rounded-[30px] border border-brand-cyan/30 shadow-[0_0_40px_rgba(0,0,0,0.5)] z-20"
                >
                  <h3 className="text-2xl font-black text-brand-cyan mb-6 italic">دورك للتحدي!</h3>
                    ...
                    <div className="flex flex-col gap-3">
                       ...
                       <div className="flex gap-3">
                          <input 
                            ...
                            className="w-32 bg-zinc-900 border-2 border-brand-cyan/20 p-4 rounded-xl text-center text-2xl font-black text-white focus:border-brand-cyan outline-none transition-all"
                          />
                          <button 
                            onClick={placeBid}
          ...
          {state.status === 'naming' && (
            ...
            >
              <div className="flex justify-between items-center mb-12">
                <div className="bg-brand-black/70 p-8 rounded-3xl border border-brand-cyan/20 w-64">
                  <Timer className="w-12 h-12 text-brand-cyan mx-auto mb-2" />
                ...
                <div className="bg-brand-black/70 p-8 rounded-3xl border border-brand-cyan/20 w-64">
                  <CheckCircle2 className="w-12 h-12 text-brand-cyan mx-auto mb-2" />
                  <span className="text-6xl font-black font-mono">{state.currentCount}</span>
                </div>
              </div>

              <div className="bg-brand-black/70 border border-brand-cyan/10 p-12 rounded-[40px] min-h-[300px] flex flex-wrap gap-4 justify-center content-start relative">
                <AnimatePresence>
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-full max-w-md px-4 z-20">
                  <form onSubmit={submitStreamerAnswer} className="relative">
                    <input 
                      ...
                      className="w-full bg-brand-black border border-brand-cyan/30 p-4 rounded-2xl text-center text-lg font-bold focus:border-brand-cyan outline-none transition-all"
                    />
                  </form>
                </div>
              </div>
          ...
          {state.status === 'review' && (
            ...
            >
              ...
              <div className="bg-brand-black/70 border border-brand-cyan/10 p-8 rounded-[30px] max-w-4xl mx-auto">
              ...
              <div className="flex items-center justify-center gap-6">
                <button
                  ...
                  className="bg-emerald-500 hover:bg-emerald-400 text-brand-black font-black px-12 py-5 rounded-2xl text-2xl shadow-[0_0_20px_rgba(16,185,129,0.3)]"
                >
                  نجاح ✅
                </button>
                <button
                  onClick={() => decideOutcome(false)}
                  className="bg-red-500 hover:bg-red-400 text-brand-black font-black px-12 py-5 rounded-2xl text-2xl shadow-[0_0_20px_rgba(239,68,68,0.3)]"
                >
                  فشل ❌
                </button>
              </div>
          ...
          {state.status === 'game_over' && (
            <motion.div 
              ...
              className="text-center space-y-8"
            >
              <Trophy className="w-48 h-48 text-brand-cyan mx-auto animate-bounce glow-cyan" />
        ...
        {state.currentMatch && state.status !== 'waiting' && state.status !== 'game_over' && (
          <div className="absolute top-24 left-8 z-50 flex flex-col items-start gap-4 pointer-events-auto">
            {/* Matchup Card */}
            <div className="bg-brand-black/90  border border-brand-cyan/20 p-4 rounded-2xl flex items-center gap-6 shadow-2xl skew-x-[-10deg] hover:skew-x-0 transition-transform duration-300 group">
                <div className="flex flex-col items-center skew-x-[10deg] group-hover:skew-x-0 transition-transform">
                  ...
                  <div className={`text-xl font-black ${state.currentMatch[0] === state.turn ? 'text-brand-cyan animate-pulse' : 'text-white'}`}>
                ...
                <div className="flex flex-col items-center skew-x-[10deg] group-hover:skew-x-0 transition-transform">
                  ...
                  <div className={`text-xl font-black ${state.currentMatch[1] === state.turn ? 'text-brand-cyan animate-pulse' : 'text-white'}`}>
      ...
      <div className="w-[400px] flex flex-col gap-6 shrink-0">
        ...
        <div className="bg-brand-black/80 rounded-[30px] border border-brand-cyan/20 flex flex-col shrink-0 overflow-hidden max-h-[35%] shadow-2xl">
          <div className="p-4 border-b border-brand-cyan/10 bg-brand-cyan/5 flex items-center justify-between">
            ...
            {state.players.map(p => (
              <div 
                ...
                className={`p-3 rounded-xl border flex items-center justify-between transition-all ${
                  !p.isEliminated 
                    ? 'bg-brand-black/70 border-brand-cyan/20 shadow-sm' 
              ...
              >
                ...
                {!p.isEliminated && state.status === 'matchmaking' && state.currentMatch?.includes(p.id) && (
                  <Swords className="w-4 h-4 text-brand-cyan animate-pulse" />
                )}
              </div>
            ))}
